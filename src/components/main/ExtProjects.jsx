...
const ExtProjects = () => {
    ...
    return <div className="my-5 py-5">
        <div className="external-links d-flex flex-column justify-content-center align-items-center my-5 p-5">
            <h3 className="external-links-title fs-1 my-5">
            ...
            <div className="links-container container w-75 flex-wrap d-flex justify-content-center align-items-center">
                {cardsInfo && cardsInfo.map((item, idx) => {
                    return (<div className="card my-4 mx-4 project-cards " key={`card-key-${idx}`}>
                        <img src="https://cdn-icons-png.flaticon.com/512/3676/3676281.png" className="card-img-top" alt={item.title} />
                        <span className="mb-3 text-wrap">{item.title}</span>
                        <a className="btn btn-dark" href={item.urlLink} target="_blank" rel="noopener noreferrer">Check me out</a>
